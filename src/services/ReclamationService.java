...
public class ReclamationService {
	...
	public void modifier(Reclamation t) {
		   try {
            String requete = "UPDATE user SET objet=?, desc_r=?, status=? WHERE idr=?";
            PreparedStatement pst = cnx.prepareStatement(requete);
	...
	public List<Reclamation> search(String x,String y) {
        ...
        try {
            String requete = "SELECT * FROM Reclamation where ( username like ? or email like ? or role like ? ) and user_id = "+session.getSession().getId();
            PreparedStatement pst = cnx.prepareStatement(requete);
			pst.setString(1, "%"+x+"%");
			pst.setString(2, "%"+x+"%");
			pst.setString(3, "%"+x+"%");
